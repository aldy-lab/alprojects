...
# Every selector is scoped to `svg.dwg`, the drawing's own root.
#
# Not tidiness -- a requirement. A <style> inside an SVG that is INLINED into a
# page applies to the whole document, so a bare `path, circle, rect { fill:
# none; stroke-width: 1.2 }` would restyle the site's icon sprite, the
# projection symbol in the footer and every other vector on the page. The
# drawings have to be inlined rather than referenced with <img>, because
# prefers-reduced-motion does not reach an SVG inside an <img> -- measured:
# 12064 pixels still moving under `reduce` through <img>, 0 when inlined. So
# the scope is what makes the reduced-motion promise keepable.
BASE = """
    svg.dwg path, svg.dwg circle, svg.dwg rect,
    svg.dwg ellipse, svg.dwg polygon, svg.dwg polyline, svg.dwg line {
      fill: none; stroke-width: 1.2; stroke-linecap: round; stroke-linejoin: round;
    }
    svg.dwg .l  { stroke: rgba(@INK@, 0.50); }
    svg.dwg .f  { stroke: rgba(@INK@, 0.22); }
    svg.dwg .ff { stroke: rgba(@INK@, 0.13); }
    svg.dwg .dash    { stroke-dasharray: 5 7; }
    svg.dwg .dashdot { stroke-dasharray: 14 5 2 5; }
    svg.dwg .fill  { fill: rgba(@INK@, 0.10); stroke: none; }
"""

# Every animated class is listed here so one media query stops all of them.
REDUCED = """
    @media (prefers-reduced-motion: reduce) {
      svg.dwg [class*="anim"] { animation: none !important; }
    }
"""
...
def _scope(css):
    """Prefix every top-level selector in one drawing's CSS with `svg.dwg`.

    BASE and REDUCED are written scoped by hand; the per-subject blocks are
    not, and there are fourteen of them. Rather than ask each subject to
    remember, the scope is applied here.

    The only thing this must not touch is the inside of an @keyframes block,
    whose "0%", "45%, 62%" and "to" are not selectors. Brace depth tells them
    apart: at depth 0 a run of text before a "{" is a selector list, inside an
    at-rule it is not.
    """
    out = []
    i = 0
    ...
    while i < len(css):
        ...
        if ch == "{":
            ...
            if depth == 0 and sel.startswith("@"):
                at_rule_depth = depth
                out.append(buf)
            elif depth == 0 or (at_rule_depth is not None and depth == at_rule_depth + 1
                                and not sel.startswith("@")):
                # inside @keyframes the "selector" is a percentage -- leave it
                if at_rule_depth is not None and sel and sel[0].isdigit() or sel in ("to", "from"):
                    out.append(buf)
                else:
                    lead = buf[:len(buf) - len(buf.lstrip())]
                    parts = [p.strip() for p in sel.split(",") if p.strip()]
                    out.append(lead + ", ".join(
                        p if p.startswith("svg.dwg") else "svg.dwg " + p for p in parts))
            else:
                out.append(buf)
            out.append("{")
            depth += 1
            ...
            depth -= 1
            if at_rule_depth is not None and depth <= at_rule_depth:
                at_rule_depth = None
            buf = ""
        else:
            buf += ch
...
def svg(parts, css):
    ...
    # role="img" with an empty label announced an unlabelled image on every
    # page. These carry no information a reader needs -- the service's own
    # heading and lead say what the drawing shows -- so they are decoration and
    # say so.
    return ('<svg xmlns="http://www.w3.org/2000/svg" class="dwg" viewBox="0 0 %d %d"'
